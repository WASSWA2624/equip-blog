...
| --- | --- | --- | --- |
| Admin authentication, session hashing, expiry, logout, and audit trail | Pass | [src/lib/auth/index.js](../../src/lib/auth/index.js), [src/lib/auth/api.js](../../src/lib/auth/api.js), [src/middleware.js](../../src/middleware.js), [06-authentication.md](./06-authentication.md) | Passwords are hashed with `scrypt`, session tokens are stored as SHA-256 hashes, cookies are `httpOnly`, expiry invalidates sessions, and auth events are written to `AuditEvent`. |
...
| Protected admin routes and protected admin APIs | Pass | [src/middleware.js](../../src/middleware.js), [src/lib/auth/config.js](../../src/lib/auth/config.js), [src/app/api](../../src/app/api/) | Middleware blocks anonymous admin access early, and route handlers independently enforce session and permission checks. |
...
- `next build` still reports the framework deprecation warning for `src/middleware.js` moving to `proxy`. This is non-blocking for Release 1 and does not invalidate the active protection logic.
- Turbopack still warns about NFT tracing through [next.config.mjs](../../next.config.mjs) and the media storage import chain rooted in [src/app/api/media/route.js](../../src/app/api/media/route.js). This is non-blocking for Release 1 and was already present before step 24.
